...
def calib_calc_fun(calib_dir, intrinsics_config_dict, extrinsics_config_dict):
    ...
    - intrinsics_config_dict: dictionary of intrinsics parameters (overwrite_intrinsics,intrinsics_method, show_detection_intrinsics, intrinsics_extension, extract_every_N_sec, intrinsics_corners_nb, intrinsics_square_size, intrinsics_marker_size, intrinsics_aruco_dict)
...
def calibrate_intrinsics(calib_dir, intrinsics_config_dict):
    ...
    for i,cam in enumerate(intrinsics_cam_listdirs_names):
        ...
        try:
            ...
            if cap.read()[0] == False:
                raise
            extract_frames(img_vid_files[0], frames_raw_dir,extract_every_N_sec, overwrite_extraction)
        ...
        if intrinsics_method == "charuco":
            ...
            if len(all_charuco_corners) > 0 and len(all_charuco_ids) > 0:
                if len(all_charuco_corners) <10:
                    ...
                    logging.info(f'Corners were detected only on {len(all_charuco_corners)} images for camera {cam}. Calibration of intrinsic parameters may not be accurate with fewer than 10 good images of the board. Please try again.')
                else:    
                    ...
                    h, w = [np.float32(i) for i in image.shape[:-1]]
                    ret.append(retval)
                    C.append(cam)
                    S.append([w, h])
                    ...
                    logging.info(f"Calibration completed for camera {cam}.")
                    logging.info(f'Intrinsics error: {np.around(retval, decimals=3)} px for camera {cam}')
                
            else:
                logging.info(f"Calibration error : None of the images identified calibration corners. Check image quality, board size parameters in config file, image extraction frequency...")
            
        #début de la méthode chessboard
        elif intrinsics_method == "chessboard":
        # find corners
            for img_path in image_files:
                if show_detection_intrinsics == True:
                    imgp_confirmed, objp_confirmed = findCorners(img_path,frames_annoted_dir, intrinsics_corners_nb, objp=objp, show=show_detection_intrinsics)
...
def calibrate_cams_all(config_dict):
    ...
    if calib_type=='convert':
        ...
        intrinsics_method = config_dict.get('calibration').get('calculate').get('intrinsics').get('intrinsics_method')
